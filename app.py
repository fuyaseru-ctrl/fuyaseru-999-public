...
with st.sidebar:
    ...
    st.divider()
    st.caption("Fuyaseru Prompt v1.4")
...
# リセットボタン
if st.button("🔄 ページを初期化（ここをクリックで更新）", type="secondary", use_container_width=True):
...
selected_char = st.selectbox(
    ...
    current_char_list,
    index=default_index,
    key=f"char_select_{st.session_state.reset_id}"
)
...
custom_char = st.text_input(
    ...
    key=f"custom_char_{st.session_state.reset_id}"
)
...
selected_q = st.selectbox(
    "質問内容を選んでください",
    current_q_list,
    key=f"q_select_{st.session_state.reset_id}"
)

custom_q = st.text_input(
    "👆 リストにない場合は自分で設定（入力すると優先されます）",
    placeholder="例：この銘柄の隠れたリスクを3つ挙げて、次の決算の注目点は？ など",
    key=f"custom_q_{st.session_state.reset_id}"
)
...
if mode == "📰 記事・ニュース分析":
    ...
    news_text = st.text_area(
        ...
        key=f"news_input_{st.session_state.reset_id}"
    )
    ...
    with st.expander("⏱️ 時間軸・ポジション設定（任意）"):
        ...
        with c1:
            time_horizon = st.selectbox(
                "時間軸", 
                ["指定なし", "デイトレ（1日）", "スイング（数日〜数週間）", "中長期（数ヶ月〜）", "超長期（10年〜）"],
                key=f"time_{st.session_state.reset_id}"
            )
        with c2:
            my_position = st.selectbox(
                "現在の状態", 
                ["指定なし", "ノンホル（未保有）", "保有中（含み益）", "保有中（含み損）", "空売り中"],
                key=f"pos_{st.session_state.reset_id}"
            )
    ...
    ticker = st.text_input(
        ...
        key=f"ticker_{st.session_state.reset_id}"
    )
    ...
    news_text = st.text_area(
        ...
        key=f"normal_news_{st.session_state.reset_id}"
    )
...
# ★ここに追加！免責事項（赤文字・小さめ）
st.caption("""
:red[**※本ツールの出力結果は、将来の投資成果を保証するものではありません。投資に係る最終決定は、必ずご自身の判断でなさいますようお願い申し上げます。**]  
:red[**また、本サービスは「フヤセル」専用の学習支援コンテンツであり、投資助言業務を行うものではありません。内容は部外秘とし、第三者への共有はお控えください。**]
""")
